...
class FrameCapture(QObject):
    ...
    def store_frame(self, frame):
        ...
        #print(f"FrameCapture: Storing frame {len(self.frames)}")
    ...
    @Slot(list)
    def capture_finished(self, framesas):
        os.makedirs(self.snapshots_folder, exist_ok=True)
        for i, frame in enumerate(framesas):
            frame=cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            cv2.imwrite(os.path.join(self.snapshots_folder, f'Snapshot_{i+1}.png'), frame)
        print(f"FrameCapture: Saved {len(framesas)} frames to {self.snapshots_folder}")
